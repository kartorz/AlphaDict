...
Section "AlphaDict" SecAlphaDict
  ...
  ${If} $LANGUAGE ==  ${LANG_SIMPCHINESE}
      CreateShortCut "$SMPROGRAMS\$StartMenuFolder\Alpha词典.lnk" "$INSTDIR\AlphaDict.exe" "" "$INSTDIR\app.ico"
  ${Else}
      CreateShortCut "$SMPROGRAMS\$StartMenuFolder\AlphaDict.lnk" "$INSTDIR\AlphaDict.exe" "" "$INSTDIR\app.ico"
  ${Endif}
